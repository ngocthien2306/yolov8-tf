...
from utils.util_tf import make_anchors
...
class Conv(keras.layers.Layer):
    ...
    def build(self, input_shape):
        ...
        # BatchNorm - PyTorch uses eps=1e-5, momentum=0.1 by default
        # TensorFlow momentum = 1 - PyTorch momentum
        self.batch_norm = layers.BatchNormalization(
            momentum=0.97,  # 1 - 0.03 (PyTorch momentum)
            epsilon=1e-3,   # PyTorch uses 1e-5, but 1e-3 is more stable in TF
            name='bn'
        )
...
class CSP(keras.layers.Layer):
    ...
    def build(self, input_shape):
        ...
        # Calculate concatenated channels: input_conv + bottleneck_conv + n * residual outputs
...
class Head(keras.layers.Layer):
    ...
    def build(self, input_shape):
        c1 = max(self.filters[0] if self.filters else 64, self.num_classes)
        c2 = max((self.filters[0] if self.filters else 64) // 4, self.dfl_channels * 4)
...
class YOLO(keras.Model):
    """Complete YOLO model in TensorFlow"""
...
def yolo_v8_tiny(num_classes: int = 1, input_size: int = 320, training_mode: bool = True):
    ...
    return YOLO(width, depth, num_classes, input_size, training_mode)
